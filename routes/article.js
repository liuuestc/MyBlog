...
router.post('/create', function (req, res) {
    if(req.session.isLogin == true){
        Poster.create({
            ...
            if (!err){
                console.log('文章创建成功！');
                var date = dbUtil.processDateString(post.createdOn);
                        ...
                        function (err, poster) {
                            if(!err){
                                ...
                                for (var i = 0; i < length ; i ++){
                                    titles += dbUtil.hotToTitle(poster[i],i);
                                }
    ...
    if (req.session.isLogin == true){
        HotPost.remove({},function (err,data) {
            if(!err){
                ...
                dbUtil.findHot('Language');      //这里由于是异步的所以不能同时返回
                dbUtil.findHot('Ideology');
                dbUtil.findHot('China');
                dbUtil.findHot('Foreign');
                res.send('<h2>创建成功！</h2>');
            } else {
                console.log('删除失败！');
                res.send('<h2>创建失败</h2>')
            ...
            if(!err){
                ...
                for (var i =0 ;i < num; i++){
                    switch (data[i]['subject']){
                        case 'Language' :
                            titles[0] = dbUtil.hotToTitle(data[i],0);
                            break;
                        case 'Ideology' :
                            titles[1] = dbUtil.hotToTitle(data[i],1);
                            break;
                        case 'China':
                            titles[2] = dbUtil.hotToTitle(data[i],2);
                            break;
                        case 'Foreign':
                            titles[3] = dbUtil.hotToTitle(data[i],3);
                            break;
                    }
                }
